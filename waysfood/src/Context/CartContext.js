...
const reducer = (state, action) => {
   ...
   switch (type) {
      case "ADD_CART":
         ...
         break;
      case "REMOVE_CART":
         const findProductByIdRemove = state.carts.find((cart) => cart.id == payload.id);

         if (findProductByIdRemove) {
            const updatedCartsRemove = state.carts.map((cart) =>
               cart.id == payload.id
                  ? {
                       ...cart,
         ...
         break;
      case "DELETE_CART":
         const filteredProducts = state.carts.filter((cart) => cart.id !== payload.id);
         return {
            ...state,
