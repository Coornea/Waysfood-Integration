...
// Assets
import brandLogo from "../../../Assets/png/waysfood.png";
...
function HistoryCard({ data, handleMapDeliveryShow, setTempMenu, showAlert }) {
   ...
   const handleDate = () => {
      ...
      const timeStr = ("0" + current.getHours()).slice(-2) + ":" + ("0" + current.getMinutes()).slice(-2);
      ...
      <Card style={{ border: "none", cursor: "pointer" }} onClick={handleClick}>
         <Card.Body>
                  ...
                  <Row>
                     <Col>
                        <p className="heading font-weight-bold mb-1 h5">
                           {role === "partner" ? userOrder.fullName : partnerOrder.fullName}
                        </p>
                        {handleDate()}
                     </Col>
                  </Row>
            ...
            <Row className="mt-4">
               ...
               <Col xs={6} md={6} className="pl-5 pl-sm-5" style={{ fontSize: "0.9em" }}>
                  {handleStatus()}
               </Col>
            </Row>
         </Card.Body>
